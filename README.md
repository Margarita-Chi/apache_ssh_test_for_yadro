...
- Linux
- Docker и Docker Compose(v2 и выше)
...
```bash
LOG_CHECK_MINUTES=10 ./run_tests_console.sh
```

Если у вас есть внешний сервер Apache + SSH, вы можете заменить TARGET_HOST на его адрес (или IP)

```bash
TARGET_HOST=192.168.1.50 SSH_USER=admin SSH_PASSWORD=secret ./run_tests_console.sh
```
...
Тесты разделены по категориям smoke, apache

Запускать тесты можно через -m:

```bash
docker compose run --rm agent pytest -m smoke -v
```
